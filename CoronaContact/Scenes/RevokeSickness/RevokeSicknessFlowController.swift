...
class RevokeSicknessFlowController {
    ...
    func revokeSickness() {
        ...
        var target: InfectionWarningType = .green
        if UserDefaults.standard.isProbablySick {
            target = .yellow
        }

        infectionWarnings = (try? cryptoService.createRevokeInfectionMessages(to: target).get()) ?? []
        ...
        flow = .statusReport
        infectionInfo = InfectionInfo(uuid: tanUUID, infectionMessages: infectionMessages)
    }
